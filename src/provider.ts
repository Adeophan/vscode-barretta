...
export class BarrettaViewProvider implements vscode.WebviewViewProvider {
          ...
          (async () => {
            if (this._view) {
              ...
              try {
                ...
                vscode.window.showInformationMessage(
                  "Barretta: There is an issue with barretta-launcher.json."
                );
  ...
  private _getHtmlForWebview(webview: vscode.Webview) {
    ...
    if (rootPath !== undefined) {
      ...
      if (fs.existsSync(jsonPath)) {
        try {
          const jsonData: LaunchJson = JSON.parse(
...
const setRootPathAsWebView = () => {
  ...
  if (!vscode.window.activeTextEditor) {
    if (vscode.workspace.workspaceFolders === undefined) {
      console.log(
        `Barretta: The target could not be identified because the folder was not opened.`
      );
      return undefined;
    }
    ...
    if (vscode.workspace.getWorkspaceFolder(activeEditorPath) !== undefined) {
      ...
      console.log(
        `Barretta: ${rootPath} was selected from the current active editors.`
      );
      return rootPath;
    } else {
